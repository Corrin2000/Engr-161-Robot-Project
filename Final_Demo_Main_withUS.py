...
try:
    while finished == 0:
        ...
        UsSensor = grovepi.digitalRead(us_port)
        
        print("Left = %d Right: %d Hall: %d"
            % (leftValue, rightValue, hallValue))
        
        if(UsSensor > UsCutoff):
            mvmt.move(spd)
        elif(UsSensor <= UsCutoff):
            mvmt.move(0)
        ...
        if((hallValue > hallMax or 
                    hallValue < hallMin) 
                    and onBeacon == 0):
                beaconCount += 1
                onBeacon = 1
        if(cargoDropped == 0):
            if(beaconCount == targetBeacon - 1):
                seekingDrop = 1
            elif(beaconCount == targetBeacon):
                mvmt.move(0)
                mvmt.forwardSeeking(spd, 25,
